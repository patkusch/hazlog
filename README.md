...
[![Tests](https://img.shields.io/badge/tests-63_passing-2ea043?style=for-the-badge)](./test)
...
HAZLOG found this with a local model (gemma3:12b, 6 min 53 s on an Apple M5 with 16 GB) and wrote it up as a DCB0160 hazard log entry. DCB0160 is the NHS standard that makes a hospital list every way a new clinical system could harm a patient; the hazard log is that list. This is the entry from [`docs/runs/gemma3-12b/hazard-log.json`](./docs/runs/gemma3-12b/hazard-log.json), as the model wrote it except for one figure: the model proposed severity Considerable, and a code rule then raised it to Major, for the reason in the entry's own rationale:
...
| Model's own `cited_requirements` | CLIN-11-R01, CLIN-11-R02, DM-04-R03 |
| Proposed control | Review and reconcile conflicting requirements between `FLLD-CLIN-v2.md` and `FLLD-DM-v3.md`; implement a mechanism to ensure data accuracy and verification before displaying legacy allergy information. |
| Proposed owner | Clinical Safety Officer |
...
The model's own severity estimate was Considerable — it treated the panel-visibility contradiction as one input among several, the way DCB0160 4.4.1 asks for an estimate but does not say how to weigh one. It should never have scored this low: one side of the contradiction (`FLLD-DM-v3.md:32`, DM-04-R05) explicitly declares its own severity "Unknown" and keeps that record active and visible on the same panel the other document assumes is empty — an unresolved severity on a live prescribing panel is the hazard, not one fact to average against the other document's claim. `src/risk.ts` now enforces that as a floor: any hazard whose own cited evidence includes a document making that declaration is never scored below Major, whatever the model proposed, and `test/severity-floor.test.ts` pins it.

The model's own `causes` for this entry still never mention DM-04-R05, and its `cited_requirements` — the field added specifically so this could be checked, not just eyeballed — confirms it: CLIN-11-R01, CLIN-11-R02 and DM-04-R03, and nothing else. That is why `severity_floor_citation_gap: true` and `severity_floor_missing_requirement_id: "DM-04-R05"` appear on this entry in the committed JSON. This is not a new bug: it is the same detection gap the previous fix left open, now made structurally checkable instead of something you had to read the prose to notice — and, on this retry, the model still did not close it on its own. It is not that the model can never make this connection: `HZ-002` in the same run (`Defaulted Allergy Severity`) cites DM-04-R05 directly, and its severity floor raise carries no gap flag. See [Live runs](#live-runs).
...
Pass 3 is also asked for `cited_requirements`: the specific requirement, row or message id each `causes` statement is actually based on, alongside the prose. This exists so a hazard's own explanation of *why* can be checked against something more precise than a paragraph a human has to parse — including, in code, whether it names the exact requirement the severity floor below fired on. See [Live runs](#live-runs) and [Limits](#limits).
...
corpus/                       local, never leaves the machine
  ...
  Pass 3  hazard log entry                Gemma, local, schema-constrained
        |   causes, cited_requirements, clinical effect, proposed severity
        |   and likelihood, controls, owner, evidence
        ...
        |   -> citation check, code (src/risk.ts): when the floor fires,
        |      does the model's own cited_requirements actually name the
        |      requirement that triggered it? If not, the entry is flagged
        |      severity_floor_citation_gap, not silently patched
        v
...
|---|---|---|
| Wall clock, three passes | 99 s | 6 min 53 s |
...
| Planted defects found (of 5) | 1, loosely | 2 of 5, via 3 findings: the panel-visibility/severity-default defect (split into two findings, HZ-001 and HZ-002 below) and the unowned safeguarding decision |
...
| Headline hazard's `cited_requirements` names the floor's trigger (DM-04-R05)? | Yes (HZ-002) | No (HZ-001) - flagged `severity_floor_citation_gap` |
...
What the 12B run got right is the point of the build: it found the panel-visibility contradiction at the top of this page unprompted, and nothing made up survived to the page. This run also split what the fixture treats as one planted defect into two separate findings: HZ-001 (the panel-visibility contradiction, CLIN-11-R01/R02 vs DM-04-R03) and HZ-002 (the severity-elicitation contradiction, CLIN-11-R05 vs DM-04-R05) — a live model does not always group things the way the fixture's author did, and this run did not surface the *No known allergy* fallback or the ward-code orphan that an earlier archived run found; that variance is expected of a real local model and is reported rather than smoothed over. What HZ-001 still misses is the detection gap this whole change exists to make checkable rather than close: its `cited_requirements` are CLIN-11-R01, CLIN-11-R02 and DM-04-R03 - DM-04-R05 is not among them, so `severity_floor_citation_gap: true` is set on the committed entry. The model is not incapable of making this connection: HZ-002 in the same run cites DM-04-R05 directly and carries no gap flag. That the same run does both - correctly connects the fact in one finding and never mentions it in another about the same underlying requirement - is exactly why this needed a structural check rather than trusting the prose.

The severity floor fixes one thing: calibration once a hazard is found. It fired on three of the four entries above (12B's HZ-001 and HZ-002, and 4B's HZ-002, "Unknown Severity Default"). All three are now Major, with the reason recorded in the entry's `severity_rationale`. It is the only calibration rule in the pipeline: every other severity and every likelihood value below is still exactly what the model proposed, unaudited. Whether the model's own `cited_requirements` name the requirement the floor actually fired on is now checked separately for every entry the floor raises (`checkFloorCitation` in `src/risk.ts`); it narrows the detection gap from silent to visible, it does not close it - see [Limits](#limits).
...
- **The model's own explanation doesn't always name the fact the score depends on - now it's checkable instead of silent.** Pass 3 asks the model for `cited_requirements` alongside `causes`: the specific requirement/row/message id each cause is actually based on. When the severity floor raises a hazard, `checkFloorCitation` (`src/risk.ts`) checks, from the corpus itself, whether the id that triggered the floor is among the model's own `cited_requirements`. If it is not, the entry is stamped `severity_floor_citation_gap: true` with `severity_floor_missing_requirement_id`, and nothing is injected into `causes` or `cited_requirements` on the model's behalf. This narrows the gap, it does not close it: on the retry archived under `docs/runs/gemma3-12b/`, HZ-001's own `causes` still never mention DM-04-R05 and the entry is flagged accordingly, while HZ-002 in the *same run* does cite DM-04-R05 and carries no flag - the same model, in the same pass, connects the fact in one finding and misses it in another. Pinned by `test/severity-floor.test.ts` (`checkFloorCitation` / `requirementIdAt`, both the gap and no-gap cases, against a synthetic corpus and the real one).
- **The fixture is the default view, not the only one.** `out/hazard-log.json` was authored to show all five planted defects. The live runs in `docs/runs/` are what the local model actually produces and vary run to run: the 12B run archived now finds two of the five, split across three findings (an earlier run found three of five via a different split). Both are held to the same tests: every quote resolves to its line, every relationship finding spans two documents.
- **Verified means the quote is there, not that it supports the claim.** An earlier 12B run's HZ-003 (ward-code orphan dependency) reasoned correctly from Row 12 of the mapping sheet and two chat lines, then cited line 41 of the migration design - the paediatric verification flag, which says nothing about ward codes. The line was real, the quote verbatim, the citation verified, and it was the wrong evidence. Provenance checking catches fabrication; it does not catch irrelevance, and nothing here changes that - a differently-run live pass, like the one archived now, can just as easily cite something real and wrong on a different hazard. That is a reviewer's job and the UI puts the line in front of them for that reason.
